        ...
        <form method="POST" action="{{ route('mesas.update', $mesa) }}">
            ...
            <div class="mb-3">
                ...
                <select name="tipo" id="tipo" class="form-select" required>
                    <option value="mesa" {{ old('tipo', $mesa->tipo) == 'mesa' ? 'selected' : '' }}>Mesa</option>
                    <option value="barra" {{ old('tipo', $mesa->tipo) == 'barra' ? 'selected' : '' }}>Barra</option>
                    <option value="terraza" {{ old('tipo', $mesa->tipo) == 'terraza' ? 'selected' : '' }}>Terraza</option>
                </select>
            </div>
                ...
                <select name="zona_id" id="zona_id" class="form-select" required>
                    @foreach ($zonas as $zona)
                        <option value="{{ $zona->id }}"
                            {{ old('zona_id', $mesa->zona_id) == $zona->id ? 'selected' : '' }}>
